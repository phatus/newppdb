    ...
    return (
        <>
            ...
            */}
            ...
            <aside className={`fixed top-0 left-0 z-50 h-screen w-64 flex-col border-r border-slate-200 bg-white dark:bg-[#1c2936] dark:border-slate-800 transition-transform duration-300 lg:translate-x-0 lg:static lg:flex ${isMobileMenuOpen ? "translate-x-0" : "-translate-x-full"}`}>
                <div className="flex h-full flex-col justify-between p-4">
                    <div className="flex flex-col gap-4">
                        {/* User Profile */}
                        <div className="flex items-center gap-3 pb-4 border-b border-slate-100 dark:border-slate-700">
                            <div className="relative size-10 rounded-full overflow-hidden shadow-sm ring-2 ring-slate-100 dark:ring-slate-700">
                                {user.image ? (
                            ...
                            <div className="flex flex-col overflow-hidden">
                                ...
                                <p className="text-slate-500 dark:text-slate-400 text-[10px] font-medium">{user.role}</p>
                            </div>
                        </div>
                        ...
                        <nav className="flex flex-col gap-1">
                            {navItems.map((item) => (
                                <Link
                                    key={item.name}
                                    href={item.href}
                                    className={`flex items-center gap-3 px-3 py-2 rounded-lg transition-colors group ${isActive(item.href)
                                        ? "bg-primary/10 text-primary dark:bg-primary/20 dark:text-primary"
                                        : "text-slate-600 hover:bg-slate-50 dark:text-slate-400 dark:hover:bg-white/5"
                                        }`}
                                    onClick={() => setIsMobileMenuOpen(false)}
                                >
                                    <span className={`material-symbols-outlined text-[20px] transition-colors ${!isActive(item.href) && "group-hover:text-primary"}`}>
                                        {item.icon}
                                    </span>
                                    <p className={`text-xs font-semibold leading-normal ${!isActive(item.href) && "group-hover:text-slate-900 dark:group-hover:text-white"}`}>
                                        {item.name}
                                    </p>
                                </Link>
                            ))}
