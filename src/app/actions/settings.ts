...
// Upload Juknis PDF
export async function uploadJuknis(formData: FormData) {
    try {
        ...
        if (file.type !== "application/pdf") {
            return { success: false, error: "Format file tidak didukung. Hanya file PDF yang diperbolehkan." };
        }
        ...
        if (first) {
            // Delete old juknis file if it exists
            if (first.juknisFile && first.juknisFile !== fileUrl) {
        ...
        await logActivity("UPLOAD_JUKNIS", "SETTINGS", first?.id || "create", `Uploaded juknis: ${file.name}`);

        return { success: true, url: fileUrl };

    } catch (error) {
        console.error("Error uploading juknis:", error);
        return { success: false, error: "Gagal mengupload file juknis" };
    }
}
