...
export default async function MassPrintPage() {
    // Fetch all verified students with documents
    ...
    return (
        <div className="flex flex-col h-full bg-background-light dark:bg-background-dark p-6 lg:px-12 lg:py-8 print:p-0 print:bg-white">
            <div className="max-w-[1100px] mx-auto space-y-6 w-full print:max-w-none print:w-full">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 print:hidden">
                    <div>
                        ...
                        <p className="text-slate-500 dark:text-slate-400 text-sm">
                            {students.length} siswa terverifikasi — Gunakan kertas A4 untuk mencetak.
                        </p>
                    </div>
                    <PrintButton />
                </div>
                ...
                {students.length === 0 && (
                    <div className="bg-amber-50 border border-amber-200 text-amber-800 p-8 rounded-xl text-center print:hidden">
                        <span className="material-symbols-outlined text-4xl mb-2">info</span>
                        <h2 className="text-xl font-bold mb-2">Belum Ada Siswa Terverifikasi</h2>
                        <p>Tidak ada siswa dengan status terverifikasi untuk dicetak kartu ujiannya.</p>
                    </div>
                )}
                ...
                {students.length > 0 && (
                    <div className="bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 rounded-xl overflow-x-auto print:bg-white print:border-none print:p-0 print:overflow-visible">
                        <div id="printable-area" className="flex flex-col gap-8 print:gap-0">
                            {students.map((student: any, studentIdx: number) => (
                                <div
                                    key={student.id}
